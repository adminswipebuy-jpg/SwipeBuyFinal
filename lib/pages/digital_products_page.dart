  ...
  Future<void> _publish() async {
    ...
    await showDialog(context: context, builder: (context) => StatefulBuilder(builder: (_, setModal) => AlertDialog(
      ...
      content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        DropdownButtonFormField<String>(initialValue: category, items: const ['Templates','Courses','Ebooks','Presets','Guides','Other'].map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: (v) => setModal(() => category = v ?? category)),
        DropdownButtonFormField<String>(initialValue: type, items: const ['PDF','ZIP','Video course','Audio','Design preset','Other'].map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: (v) => setModal(() => type = v ?? type)),
        const SizedBox(height: 8),
