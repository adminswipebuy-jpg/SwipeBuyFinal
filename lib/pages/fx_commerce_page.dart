  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(18), children: [
        ...
        DropdownButtonFormField<String>(value: displayCurrency, decoration: const InputDecoration(border: OutlineInputBorder(), prefixIcon: Icon(Icons.payments_outlined), labelText: 'Preferred display currency'), items: currencies.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => setState(() => displayCurrency = v ?? displayCurrency)),
        ...
        Row(children: [
          Expanded(child: DropdownButtonFormField<String>(value: from, decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'From'), items: currencies.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => setState(() => from = v ?? from))),
          const SizedBox(width: 10),
          const Icon(Icons.swap_horiz_rounded),
          const SizedBox(width: 10),
          Expanded(child: DropdownButtonFormField<String>(value: to, decoration: const InputDecoration(border: OutlineInputBorder(), labelText: 'To'), items: currencies.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => setState(() => to = v ?? to))),
        ]),
        const SizedBox(height: 10),
